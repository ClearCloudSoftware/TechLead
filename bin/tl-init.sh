...
case "$harness" in
  claude)   worker="$TL_HOME/adapters/claude-worker.sh";   grill="$TL_HOME/adapters/claude-grill.sh"
            propose="$TL_HOME/adapters/claude-grill-propose.sh"; scaffold="$TL_HOME/adapters/claude-scaffold-test.sh"; kick="$TL_HOME/adapters/claude-kickoff.sh"
  ...
  opencode) worker="$TL_HOME/adapters/opencode-worker.sh"; grill="$TL_HOME/adapters/opencode-grill.sh"
            propose=""; scaffold=""; kick=""; specdiff=""; standards=""; answer=""   # no opencode adapters yet → those axes fall back
...
mkdir -p "$(dirname "$TL_CONFIG")"
{
  ...
  [ -n "$kick" ] && printf 'export TL_KICKOFF_CMD="${TL_KICKOFF_CMD:-%s}"\n' "$kick"
